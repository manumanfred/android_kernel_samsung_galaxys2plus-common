...
#define CAL_RPC_DEVID	 9	/* SERIAL_DEVICE_RPC_0 - Same as dft ATC RPC ch */
 ...
 */
typedef struct {
	struct file *mUserfile;	/* user file handle (for open, close and ioctl calls) */
	int ep;			/* log enable/disable status bits */
	UInt8 clientId;
} RpcIpc_PrivData_t;
	...
	case RPC_PKT_ALLOC_BUFFER_PTR_IOC:
		{
			RPC_READ_LOCK;
			retVal =
			    handle_pkt_alloc_buffer_ptr_ioc(filp, cmd, arg);
			RPC_READ_UNLOCK;
			break;
		}
...
/*	if(unfreedMsgs > 0)
	...
	if (queuedMsgs > 0 || unfreedMsgs > 0) {
		_DBG(RPC_TRACE
		     ("k:RPC_ServerRecoverUnfreedMsgsForClient FORCE Free Msgs tid %d cid %d, pid %d name %s qp=%d up=%d\n",
...
RPC_Result_t RPC_ServerDispatchMsg(PACKET_InterfaceType_t interfaceType,
				   ...
				   PACKET_BufHandle_t dataBufHandle,
				   UInt16 msgId, Boolean broadcastMsg)
{
	RpcCbkElement_t *elem;
	RpcClientInfo_t *cInfo;
	...
	if (cInfo->tid && RpcIsTaskValid(cInfo->tid) == -ESRCH) {
		...
		_DBG(RPC_TRACE
		     ("k:RPC_ServerDispatchMsg ERROR(Invalid tid %d) cid %d, pid %d name %s\n",
		      cInfo->tid, clientId, cInfo->pid, cInfo->pidName));
	...
	if (cInfo->notresponding) {
		...
		if (!empty) {
			_DBG(RPC_TRACE
			     ("k:RPC_ServerDispatchMsg ERROR( NOT RESPONDING tid %d) cid %d, pid %d name %s\n",
			      cInfo->tid, clientId, cInfo->pid,
			      cInfo->pidName));
			return RPC_RESULT_ERROR;
		}
	}
	elem =
	    kmalloc(sizeof(RpcCbkElement_t),
	...
	RpcDbgUpdatePktStateEx((int)dataBufHandle, PKT_STATE_NA, clientId,
			       PKT_STATE_CID_DISPATCH, 0, (int)msgId,
			       (UInt8)interfaceType);
	HISTORY_RPC_LOG("Dispatch", clientId, (int)dataBufHandle, interfaceType,
			msgId);
	spin_lock_bh(&cInfo->mLock);
	list_add_tail(&elem->mList, &cInfo->mQ.mList);
	cInfo->availData = 1;
	...
	if (copyRc != 0) {
		...
		_DBG(RPC_TRACE
		     ("  src addr=%x, dest addr=%x, size=%d\n", (int)&ioc_param,
		      (int)param, sizeof(rpc_pkt_avail_t)));
		_DBG(RPC_TRACE
				     ...
				     UInt32 param)
{
	...
	if (!pktElem) {
		_DBG(RPC_TRACE
		     ("k:handle_pkt_rx_buffer_ioc Allocation error cid=%d\n",
		      ioc_param.clientId));
		return -EFAULT;
	}
	...
	HISTORY_RPC_LOG_PKT("rxBuf", ioc_param.clientId,
			    (int)Item->dataBufHandle);

	RpcDbgUpdatePktStateEx((int)Item->dataBufHandle, PKT_STATE_NA,
			       ioc_param.clientId, PKT_STATE_CID_FETCH, 0, 0,
			       0xFF);
				       ...
				       UInt32 param)
{
	...
	/*fixme: Handle return value? */
	{
		...
		HISTORY_RPC_LOG_PKT("Send", ioc_param.clientId,
				    (int)ioc_param.dataBufHandle);
	}
...
static long free_client_pkt(struct file *filp, UInt8 clientId,
	...
	list_for_each_safe(listptr, pos, &cInfo->pktQ.mList) {
		...
		_DBG(RPC_TRACE
		     ("k:free_client_unfreed_pkts FREE clientID %d pkt %d res %d\n",
		      (int)clientId, (int)Item->dataBufHandle, (int)res));
		kfree(Item);
		ret = (res == RPC_RESULT_OK) ? 0 : -EFAULT;
	}
	spin_unlock_bh(&cInfo->mLock);
				       ...
				       UInt32 param)
{
	...
	//Coverity [TAINTED_SCALAR]
	ret =
	    free_client_pkt(filp, ioc_param.clientId, ioc_param.dataBufHandle);
			       ...
			       UInt32 param)
{
	...
	HISTORY_RPC_LOG("Cmd", 0, (int)ioc_param.dataBufHandle,
			ioc_param.input1, ioc_param.outParam);
					     ...
					     unsigned int cmd, UInt32 param)
{
	...
	HISTORY_RPC_LOG("Register", clientId, 0, current->tgid,
			cInfo->info.interfaceType);
					       ...
					       unsigned int cmd, UInt32 param)
{
	...
	HISTORY_RPC_LOG("DeRegister", cInfo->clientId, 0, current->tgid,
			cInfo->info.interfaceType);
						...
						unsigned int cmd, UInt32 param)
{
	...
	HISTORY_RPC_LOG("RegisterEx", clientId, 0, current->tgid,
			cInfo->info.interfaceType);
				   ...
				   UInt32 param)
{
	...
	if (!(ioc_param.isEmpty)) {
		...
		if (!pktElem) {
			_DBG(RPC_TRACE
			     ("k:handle_pkt_poll_ex_ioc Allocation error cid=%d\n",
			      ioc_param.clientId));
			return -EFAULT;
		}
		...
		if (Item->dataBufHandle) {
			ioc_param.bufInfo.bufferLen =
			    RPC_PACKET_GetBufferLength(Item->dataBufHandle);
			...
			ioc_param.offset =
			    IPC_SmOffset(ioc_param.bufInfo.buffer);
					    ...
					    UInt32 param)
{
	...
	HISTORY_RPC_LOG("Alloc", 0, (int)ioc_param.pktBufHandle,
			ioc_param.offset, ioc_param.allocatedSize);
					  ...
					  UInt32 param)
{
...
#ifdef USE_INTERNAL_BUFFER
	{
		...
		if (copy_from_user
		    (buffer, ioc_param.txBuf.buffer,
		     ioc_param.txBuf.bufferLen) != 0) {
			_DBG(RPC_TRACE
	...
	/*fixme: Handle return value? */
	{
		RPC_PACKET_SetBufferLength(ioc_param.txBuf.dataBufHandle,
					   ioc_param.txBuf.bufferLen);

		RPC_PACKET_SetContext(ioc_param.txBuf.interfaceType,
				      ioc_param.txBuf.dataBufHandle,
				      ioc_param.txBuf.context1);
		RPC_PACKET_SetContextEx(ioc_param.txBuf.interfaceType,
					ioc_param.txBuf.dataBufHandle,
					ioc_param.txBuf.context2);

		RPC_READ_UNLOCK;
		RPC_PACKET_SendData(ioc_param.txBuf.clientId,
				    ioc_param.txBuf.interfaceType,
				    ioc_param.txBuf.channel,
				    ioc_param.txBuf.dataBufHandle);
		RPC_READ_LOCK;

		HISTORY_RPC_LOG("SendEx", ioc_param.txBuf.clientId,
				(int)ioc_param.txBuf.dataBufHandle,
				ioc_param.txBuf.interfaceType,
				ioc_param.txBuf.context2);
				    ...
				    UInt32 param)
{
	...
	if (copy_from_user(&ioc_param, (RpcPktRegMsgIds_t *) param, 8) != 0) {
		_DBG(RPC_TRACE
	...
	readSize =
	    (offsetof(RpcPktRegMsgIds_t, msgs) + (ioc_param.msgsSize * 2));

	if (copy_from_user
	    (&ioc_param, (RpcPktRegMsgIds_t *) param, readSize) != 0) {
		_DBG(RPC_TRACE
		     ("k:handle_pkt_reg_msgs_ioc - copy_from_user() had error\n"));
		return -EFAULT;
	}

	ret = (long)rpc_register_client_msgs(cid, ioc_param.msgs,
					     ioc_param.msgsSize);
					...
					UInt32 param)
{
	...
	if (copy_from_user
	    (&ioc_param, (int *)param, sizeof(rpc_pkt_cp_reset_ack_t)) != 0) {
		_DBG(RPC_TRACE
	...
	_DBG(RPC_TRACE("handle_pkt_ack_cp_reset_ioc: %d %d\n",
		       ioc_param.rpcClientID, ioc_param.interfaceType));
	...
	if (INTERFACE_CAPI2 != ioc_param.interfaceType) {
		...
		if (bComplete) {
			...
			RPC_PACKET_FilterAckReadyForCPReset(ioc_param.
							    rpcClientID,
							    ioc_param.
							    interfaceType);
		}
...
void RpcDumpTaskState(RpcOutputContext_t *c, pid_t inTid, pid_t inPid)
{
	...
	if (t) {
		RpcDbgDumpStr(c, "\t%s pid:%d tid:%d state:%d flag:0x%x\n",
			      t->comm, inPid, inTid, (int)t->state,
			      (int)t->flags);

		RpcDumpTaskCallStack(c, t);
	} else
		RpcDbgDumpStr(c,
			      "\tError!!!(Process not found) pid:%d tid:%d\n",
			      inPid, inTid);
}
...
static int log_seq_show(struct seq_file *s, void *v)
{
	...
	if (context->counter == 0)
		...
		ret =
		    RpcDbgDumpPktState(&(context->out), &(context->offset), 10);
	else if (context->counter == 6)
		ret =
		    RbcDbgDumpGenInfo(&(context->out), &(context->offset), 10);
