		...
		if (fsize != sizeof(SIMLOCK_NVDATA_t)) {
			...
			if (hFileTmp->f_op && hFileTmp->f_op->read) {
				...
				if (retval > 0) {
					...
					pr_err("Read SIM Lock NV data Failed!");
					pr_err(" retval:%d", retval);
					pr_err
					    (" sizeof(SIMLOCK_NVDATA_t):%d\n",
					     sizeof(SIMLOCK_NVDATA_t));
				}
			} else {
				pr_err("hFileTmp no read method Error!!!\n");
			}
		}
	} else {
	...
	if (GetSimlockNvdata(&simlock_nvdata) && (simlockFile != NULL)) {
		...
		case SEC_SIMLOCK_NETWORK_LOCK:
			...
			unlock_attempt =
			    ...
			    &Network_Unlock_Attempt : &simlock_nvdata.
		...
		case SEC_SIMLOCK_NET_SUBSET_LOCK:
			...
			unlock_attempt =
			    simlockFile->resetUnlockCounter ?
			    &Network_Subset_Unlock_Attempt : &simlock_nvdata.
		...
		case SEC_SIMLOCK_PROVIDER_LOCK:
			...
			unlock_attempt =
			    simlockFile->resetUnlockCounter ?
			    &Service_Provider_Unlock_Attempt :
		...
		case SEC_SIMLOCK_CORP_LOCK:
			...
			unlock_attempt =
			    simlockFile->resetUnlockCounter ?
			    &Corporate_Unlock_Attempt : &simlock_nvdata.
			    corporate_unlock_attempt;
...
static Boolean SIMLockCheckPHSIMLock(SimNumber_t SimId, UInt8 *imsi)
{
	...
	if (GetSimlockNvdata(&simlock_nvdata)) {
		if (simlock_nvdata.phone_lock_setting == PH_SIM_FULL_LOCK_ON) {
			...
			pr_info
			    ("%s:simlock_nvdata.phone_lock_imsi:%s imsi:%s\n",
			     __func__, (char *)&simlock_nvdata.phone_lock_imsi,
			     (char *)imsi);
			result = ((imsi != NULL)
				  &&
				  (strcmp
		...
		if (strcmp((char *)key, simlock_nvdata.phone_lock_pwd) == 0) {
			...
			if ((imsi != NULL) && (imsi[0] != '\0')) {
				strncpy((char *)&simlock_nvdata.phone_lock_imsi,
					...
					- 1);
				((char *)
				 &simlock_nvdata.phone_lock_imsi)[sizeof
			     ...
			     UInt8 *gid2)
{
	...
	if (GetSimlockNvdata(&simlock_nvdata)) {
		...
		if (Network_Lock_Status[SimId] != SIMLOCK_VERIFIED) {
			if (!simlock_nvdata.network_lock_ind || ((imsi != NULL)
								 &&
								 SIMLockCheckNetworkLock
								 (SimId,
								  imsi))) {
		...
		if (Service_Provider_Lock_Status[SimId] != SIMLOCK_VERIFIED) {
			if (!simlock_nvdata.service_provider_lock_ind
			    || ((imsi != NULL) && (gid1 != NULL)
				&& SIMLockCheckProviderLock(SimId, imsi, gid1))) {
				Service_Provider_Lock_Status[SimId] =
				    SIMLOCK_OPENED;
			} else {
				Service_Provider_Lock_Status[SimId] =
...
//
******************************************************************************/
SEC_SimLock_Status_t SIMLockChangePasswordPHSIM(UInt8 *old_pwd, UInt8 *new_pwd)
{
	SEC_SimLock_Status_t result;
	SIMLOCK_NVDATA_t simlock_nvdata;
	...
	if (GetSimlockNvdata(&simlock_nvdata)) {
		if (strcmp((char *)old_pwd, simlock_nvdata.phone_lock_pwd) == 0) {
			...
			simlock_nvdata.phone_lock_pwd[sizeof
						      (simlock_nvdata.
						       phone_lock_pwd)
						      - 1] = '\0';
...
SEC_SimLock_Status_t SIMLockSetLock(SimNumber_t SimId,
				    ...
				    SEC_SimLock_LockType_t lockType, UInt8 *key)
{
	if (lockType == SEC_SIMLOCK_PHONE_LOCK) {
		SEC_SimLock_Status_t result = SIMLockSetPHSIMLock(SimId,
								  ((action ==
								    0) ?
								   PH_SIM_LOCK_OFF
								   :
...
UInt8 SIMLockGetRemainAttempt(SimNumber_t SimId)
{
	...
	if (NULL != unlock_attempt) {
		remain_attempt =
		    (*unlock_attempt <
		     simlockFile->
		     maxUnlockAttempt ? simlockFile->maxUnlockAttempt -
		     (*unlock_attempt) : 0);
	} else {
