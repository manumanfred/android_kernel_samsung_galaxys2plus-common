...
static long handle_hdcp_aes_ioc(struct file *filp, unsigned int cmd,
						   unsigned long param)
{
	sec_hdcp_aes_data_t *ioc_param = NULL;
	...
	if (!EncDec(&ioc_param->hdcp_out_string[0],
		&ioc_param->hdcp_in_string[0],
		ioc_param->hdcp_string_len,
		ioc_param->hdcp_enc_dec
			| (ioc_param->hdcp_wrapping_key << 16))) {
		pr_err("handle_hdcp_aes_ioc: Fail to encrypt.decrypt\n");
		kfree(ioc_param);
		return -EFAULT;
	}
						   ...
						   unsigned long param)
{
	...
	if((ioc_param.hdcp_out_string == NULL) ||
		(ioc_param.hdcp_string_len > MAX_HDCP_DIGITS) ||
		(ioc_param.hdcp_string_len < AES_BLOCK_SIZE) ||
		((ioc_param.hdcp_string_len % AES_BLOCK_SIZE) != 0)) {
		pr_err
		("handle_hdcp_aes_ioc: Error parameters\n");
		return -EFAULT;
	}
	...
	if(aes_buf_vir == NULL ) {
		pr_err
		("handle_hdcp_aes_ioc: dma buffer alloc error\n");
		return -EFAULT;
	}
	...
	memcpy(&aes_v[0], &ioc_param.hdcp_in_string[0], ioc_param.hdcp_string_len);

	pr_err
		("handle_hdcp_aes_ioc: calling secure_api_call()\n");

	secure_api_call(SSAPI_ROW_AES,
				(int)&aes_p[0],
				ioc_param.hdcp_string_len,
				(int)&aes_p[ioc_param.hdcp_string_len],
				ioc_param.hdcp_enc_dec | (ioc_param.hdcp_wrapping_key << 16));

	memcpy(&ioc_param.hdcp_out_string[0], &aes_v[ioc_param.hdcp_string_len], ioc_param.hdcp_string_len);
	if(aes_buf_vir)
		dma_free_coherent(NULL, SZ_4K, aes_buf_vir, aes_buf_phy);

	pr_err
		("handle_hdcp_aes_ioc: secure_api_call() done\n");
